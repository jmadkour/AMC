...
def process_csv2excel(xls_file, csv_file, add_notes=0):
    try:
        ...
        # Supprimer la colonne 'Code' ou 'student.number' si elles existent
        for col_to_drop in ['Code', 'student.number']:
            if col_to_drop in csv.columns:
                csv = csv.drop(columns=[col_to_drop])

        # Renommer la colonne 'Mark' en 'Note' si elle existe
        if 'Mark' in csv.columns:
            csv = csv.rename(columns={'Mark': 'Note'})
